...
%>
<html>
<head><title>WashingtonHouseRabbitSociety.org News Admin</title><!--mstheme--><link rel="stylesheet" type="text/css" href="../_themes/sumipntg/sumi1011.css"><meta name="Microsoft Theme" content="sumipntg 1011, default">
</head>
<body>
<center>
<h1>News For HouseRabbit</h1>
<hr>
<%
'--------------------------------------------------------------------
' Open the connection
...
case "New"
	Call DisplayRecord(0,NULL,NULL,now(),"Adding")
case "Edit"
	sql="Select id,title,Text,date from news Where Id=" & request("Index")
	set rsJokes=Conn.execute(sql)
	Call DisplayRecord(request("Index"),rsJokes("Title"),rsJokes("Text"),rsJokes("Date"),"Updating")
case "Delete"
	sql="Select id,Title,Text,Date from news Where Id=" & request("Index")
	set rsJokes=Conn.execute(sql)
	Call DisplayRecord(request("Index"),rsJokes("Title"),rsJokes("Text"),rsJokes("Date"),"Deleting")
case "Submit New"
	...
	sql="INSERT INTO news ([id],[Title],[Date],[Text]) VALUES("
	sql=sql & iMax & ","
	sql=sql & "'" & replace(request("Title"),"'","''") & "',"
	sql=sql & "'" & request("Date") &"',"
	sql=sql & "'" & replace(request("Text"),"'","''") & "')"

	'sql="INSERT INTO news ([id],[Title],[Text]) VALUES("
	'sql=sql & iMax & ","
	'sql=sql & "'" & replace(request("Title"),"'","''") & "',"
	'sql=sql & "'" & replace(request("Text"),"'","''") & "')"
...
case "Submit Changes"
	...
	Call DisplayMenu()
case "Delete It"
	sql="DELETE FROM news WHERE [id]=" & request("Index")
	'response.write sql
	On error resume next
	Conn.execute(sql)
	If err<>0 then
...
'--------------------------------------------------------------------
Sub DisplayMenu()
	sql="Select Title,Id From news ORDER BY title"
	set rsWhatsNew=Conn.execute(sql)
%>
	<form name="FAQ" action="AdminNews.asp" method="post">
	<b>Select a news Entry</b> <select name="Index">
<%
...
' Display a record
'--------------------------------------------------------------------
Sub DisplayRecord(iIndex,sTitle,sText,sDate,sMode)
%>
	...
	<tr>
		...
		<td><input name="Date" type="text" size="30" value="<%=sDate%>"></td>
	</tr>
	<tr>
