...
case "Edit"
	sql="Select * from Adoptferrets Where Id=" & request("Index")
...
case "Submit New"
	sql="Select Max(id) as MaxId From Adoptferrets"
	set rsMax=Conn.execute(sql)
	iMax=rsMax("MaxId")+1
	set rsMax=nothing
	If isnull(iMax) then iMax=1
	sql="INSERT INTO Adoptferrets ([id],[name],[Picture1],[Picture2],[Picture3],[Picture4],[Adopted],[Archive],[Desc]) VALUES("
	sql=sql & iMax & ","
...
case "Submit Changes"
	...
	Call DisplayMenu()
case "Delete It"
	sql="DELETE FROM Adoptferrets WHERE id=" & request("Index")
	'response.write sql
	On error resume next
	Conn.execute(sql)
	If err<>0 then
...
'--------------------------------------------------------------------
Sub DisplayMenu()
	sql="Select ID,Name From Adoptferrets ORDER BY Name"
	set rsWhatsNew=Conn.execute(sql)
%>
	<form name="FAQ" action="AdminadoptFer.asp" method="post">
	<b>Select a ferret Entry</b> <select name="Index">
<%
