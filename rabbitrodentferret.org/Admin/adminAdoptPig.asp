...
%>
<html>
<head><title>RabbitRodentFerret.org Rabbits Admin</title>
...
case "Edit"
	sql="Select * from AdoptGuineas Where Id=" & request("Index")
...
case "Submit New"
	sql="Select Max(id) as MaxId From AdoptGuineas"
	set rsMax=Conn.execute(sql)
	iMax=rsMax("MaxId")+1
	set rsMax=nothing
	If isnull(iMax) then iMax=1
	sql="INSERT INTO AdoptGuineas([id],[name],[Picture1],[Picture2],[Picture3],[Picture4],[Adopted],[Archive],[Desc]) VALUES("
	sql=sql & iMax & ","
...
case "Submit Changes"
	...
	Call DisplayMenu()
case "Delete It"
	sql="DELETE FROM AdoptGuineas WHERE id=" & request("Index")
	'response.write sql
	On error resume next
	Conn.execute(sql)
	If err<>0 then
...
'--------------------------------------------------------------------
Sub DisplayMenu()
	sql="Select ID,Name From AdoptGuineas ORDER BY Name"
	set rsWhatsNew=Conn.execute(sql)
%>
	<form name="FAQ" action="AdminadoptPig.asp" method="post">
	<b>Select a Guinea Pig Entry</b> <select name="Index">
<%
	Do while not rsWhatsNew.eof
		response.write "<option value=""" & rsWhatsNew("Id") & """>" & rsWhatsNew("Name") & "</option>"
		rsWhatsNew.movenext
	loop
