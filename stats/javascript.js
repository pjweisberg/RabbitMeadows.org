...
function submitwhoisquery(registry, ipaddress)
	...
	if (registry == "LACNIC")
		{
		document.lacnic.qr.value = ipaddress;
...
function agreesetup(intType)
		{
		if (intType == 1)
			var agree=confirm("The MetaTraffic install will take a few minutes to load the nessasary data. Press OK to continue.");
		else
			var agree=confirm("Upgrading MetaTraffic from a previous version can take a long time depending on the amount of data selected to upgrade. Press OK to continue or CANCEL to change your setup options.");
		if (agree)
			return true;
		else
			return false;
...
function agreefile(form, action)
		{
		var agree=confirm("You have not selected a file to upload. By proceeding, MetaTraffic will try and use the data\\" 
		+ action + ".txt file in the metatraffic installation folder.");
		if (agree)
			return true;
		else
			return false;
...
function installform()
		...
		if (install.options[install.selectedIndex].value == 2)
			...
			showhideconfig('username',1);
			showhideconfig('password',1);
			showhideconfig('password2',1);
			}
		else
			{
			if (install.options[install.selectedIndex].value == 1)
				{
				...
				{
				showhideconfig('upgrade',0);
				showhideconfig('username',0);
				showhideconfig('password',0);
				showhideconfig('password2',0);
				}
			}
		}
